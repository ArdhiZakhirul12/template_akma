<div>
    ...
    <div class="p-5 bg-white dark:bg-zinc-700 rounded-lg shadow-md overflow-auto">
        ...
        <table id="rekap-rab"
            ...
            <tbody>
                {{-- @if ($allmonthlyTotal != null || $allMontlyTotal != []) --}}
                ...
                {{-- @endif --}}
                @if ($monthlySums != null || $monthlySums != [])

                    @foreach ($uraians as $subkategori => $items)
                        <tr class="font-bold text-white" style="pointer-events: none;">
                            <td style="background-color: #007bff; pointer-events: none;" colspan="2">
                                {{ $subkategori }}
                            </td>
                        ...
                        @if ($monthlySums != null)
                            @foreach ($items as $item)
                                <tr>
                                    ...
                                    @for ($i = 1; $i <= 12; $i++)
                                        <th
                                            class="text-center font-light {{ $monthlySums[$item->id][$i] != 0 ? 'text-amber-500' : '' }}">
